...
import requests
from beepbeep.trainingservice.database import db, Training_Objective
import json
...
def add_training_objective():
    
    training_objective = request.json
    try:
        runner_id = int(training_objective['runner_id'])
        ...
        kilometers_to_run = int(training_objective['kilometers_to_run'])
    except:
        return "",400
